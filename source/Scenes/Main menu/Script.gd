...
func _ready():
	# Wait for a moment to pass, queue free the Shapes in Hats logo, wait for another moment, make the game's logo appear, then queue it free.
	# The logos are freed from the scene tree instead of simply hiding themselves because they're no longer needed in the scene.
